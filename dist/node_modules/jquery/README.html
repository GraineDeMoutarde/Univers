...
<ol>
<li>Install <a href="https://code.google.com/p/msysgit/">msysgit</a> (Full installer for official Git) and a
<a href="http://nodejs.org">binary version of Node.js</a>. Make sure all two packages are installed to the same
location (by default, this is C:\Program Files\Git).</li>
<li>Install <a href="http://cygwin.com/">Cygwin</a> (make sure you install the git and which packages), and
a <a href="http://nodejs.org/">binary version of Node.js</a>.</li>
</ol>
...
<a href="http://mxcl.github.com/homebrew/">Homebrew</a>. Once Homebrew is installed, run <code>brew install git</code> to install git,
...
<pre><code class="language-bash">cd jquery &amp;&amp; npm install
</code></pre>
<p>Make sure you have <code>grunt</code> installed by testing:</p>
<pre><code class="language-bash">grunt -version
</code></pre>
...
<pre><code class="language-bash">git pull; git checkout $(git describe --abbrev=0 --tags)
</code></pre>
<p>Then, make sure all Node dependencies are installed and all Git submodules are checked out:</p>
<pre><code class="language-bash">npm install &amp;&amp; grunt
</code></pre>
...
<pre><code class="language-bash">grunt custom:-deprecated
</code></pre>
<p>Exclude <strong>dimensions</strong>:</p>
<pre><code class="language-bash">grunt custom:-dimensions
</code></pre>
<p>Exclude <strong>effects</strong>:</p>
<pre><code class="language-bash">grunt custom:-effects
</code></pre>
<p>Exclude <strong>offset</strong>:</p>
<pre><code class="language-bash">grunt custom:-offset
</code></pre>
<p>Exclude <strong>all</strong> optional modules:</p>
<pre><code class="language-bash">grunt custom:-ajax,-css,-deprecated,-dimensions,-effects,-offset
</code></pre>
...
<pre><code class="language-bash">cd jquery &amp;&amp; grunt watch
</code></pre>
...
<pre><code class="language-bash">grunt update_submodules
</code></pre>
...
<h3>Submodules</h3>
<p>The repository uses submodules, which normally are handled directly by the <code>grunt update_submodules</code> command, but sometimes you want to
...
<p>Or:</p>
<pre><code class="language-bash">git clone --recursive https://github.com/jquery/jquery.git
cd jquery
</code></pre>
<p>If you want to work inside a submodule, it is possible, but first you need to checkout a branch:</p>
<pre><code class="language-bash">cd src/sizzle
git checkout master
</code></pre>
...
<p>Example:</p>
<pre><code class="language-js">t(&quot;Check for something&quot;, &quot;//[a]&quot;, [&quot;foo&quot;, &quot;baar&quot;]);
</code></pre>
...
<pre><code class="language-js">testIframe( fileName, testName, callback );
</code></pre>
<p>Callback arguments:</p>
<pre><code class="language-js">callback( jQueryFromIFrame, iFrameWindow, iFrameDocument );
</code></pre>
...
<pre><code class="language-js">testIframeWithCallback( testName, fileName, callback );
</code></pre>
<h2>Questions?</h2>
